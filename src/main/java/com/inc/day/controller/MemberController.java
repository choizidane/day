...
import com.inc.day.service.MemberService;
...
public class MemberController {
	...
	public String signup(@ModelAttribute @Valid Member member, BindingResult result, HttpSession session) {
		...
		if(!member.isPasswordValid()) {
			FieldError error = new FieldError("passwordError", "passwordConfirm", "패스워드가 일치하지않습니다.");
			result.addError(error);
		}
		if(!member.getEmail().equals((String)session.getAttribute("email"))) {
			FieldError error = new FieldError("emailNotEqualsError", "email", "인증받은 메일로 입력하세요.");
			result.addError(error);
		}
		if(!member.getEmailCode().equals((String)session.getAttribute("emailCode"))) {
			FieldError error = new FieldError("emailCodeError", "emailCode", "이메일코드가 일치하지 않습니다.");
			result.addError(error);
		}
	...
	public String signin(@ModelAttribute Member member, BindingResult result, HttpServletRequest request) {
		...
		if(savedMember == null) {
			result.addError(new FieldError("notExistId", "id", "존재하지않는아이디입니다."));
				
		}else if(!savedMember.getPassword().equals(member.getPassword())) {
			result.addError(new FieldError("passwordNotSame", "password", "패스워드가일치하지않습니다."));
		}
		if(result.hasErrors()) {
			return "member/signin";
		}
